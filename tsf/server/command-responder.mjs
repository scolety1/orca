...
import { buildActionAmbiguityFallback } from './command-action-ambiguity-fallback.mjs'
import {
  respondUnroutedGlobalScope,
  formatFleetStatusText
} from './command-global-scope-bridge.mjs'
import {
...
export async function respondCommand({
  ...
  if (!DISPATCH_WORTHY_INTENTS.has(intent)) {
    ...
    if (UNROUTED_QUESTION_INTENTS.has(intent) && resolution.matches.length === 0) {
      const globalScopeResult = await respondUnroutedGlobalScope({
        message,
        decisionClass,
        projects,
        opState,
      ...
      if (globalScopeResult) {
        return globalScopeResult
      }
    }
    // Finding #4 (command-action-ambiguity-fallback.mjs).
    const ambiguityFallback = buildActionAmbiguityFallback(
      message,
      intent,
      decisionClass,
      resolvedProjectIds,
      scopeFor(resolvedProjectIds)
    )
    if (ambiguityFallback) {
      return ambiguityFallback
    }
    return {
      intent,
      decisionClass,
