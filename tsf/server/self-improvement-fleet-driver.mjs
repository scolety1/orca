...
import {
  computeRepairMissionId,
  originateRepairMission
} from './self-improvement-mission-origination.mjs'
...
import {
  recordVerifiedCorrectionLesson,
  recordVerifierFailureLesson
} from './self-improvement-learning-ledger-wiring.mjs'

const ACTIONABLE_STATUSES = Object.freeze([
...
function latestPassingVerification(checkpoint) {
  return (
    (checkpoint?.verifierResults ?? []).toReversed().find((r) => r.verdict === 'VERIFIED_PASS') ??
    null
  )
}

// One bounded action for ONE finding -- the unit of work a tick performs.
export async function advanceOneFinding(
  finding,
  { canonicalRepoPath, clock = () => new Date(), deps = {} }
) {
  if (finding.status === 'ELIGIBLE_FOR_AUTOFIX') {
    const origination = await (deps.originateRepairMission ?? originateRepairMission)(finding, {
      canonicalRepoPath,
      clock,
  ...
  if (finding.status === 'FIX_MISSION_CREATED' || finding.status === 'FIX_IN_PROGRESS') {
    const attempt = await (deps.runRepairAttempt ?? runRepairAttempt)({
      finding,
      missionId,
      canonicalRepoPath,
      clock,
      deps: deps.repairCycleDeps ?? {}
    })
    if (attempt.verification?.verdict === 'VERIFIED_FAIL') {
      await (deps.recordVerifierFailureLesson ?? recordVerifierFailureLesson)(
        finding,
        missionId,
        attempt.verification,
        clock,
        deps.learningLedgerDeps ?? {}
      )
    }
    return {
  ...
  if (!passing?.detail?.worktreePath) {
    return {
      ...
      findingId: finding.findingId,
      reason: 'READY_FOR_ADOPTION but no recorded passing verification worktree evidence found'
    }
  }
  ...
  const redogfood = adoption.adopted
    ? await runRedogfood_({
        finding,
        missionId,
        targetPath: canonicalRepoPath,
        adoptedSha: adoption.adoptedHead,
        clock,
        deps: deps.redogfoodDeps ?? {}
      })
    : await runRedogfood_({
        finding,
        missionId,
        targetPath: passing.detail.worktreePath,
        clock,
        deps: deps.redogfoodDeps ?? {}
      })

  if (redogfood.outcome === 'RESOLVED') {
    await (deps.recordVerifiedCorrectionLesson ?? recordVerifiedCorrectionLesson)(
      redogfood.finding ?? finding,
      missionId,
      clock,
      deps.learningLedgerDeps ?? {}
    )
...
// function itself.
export function startSelfImprovementFleetDriver({
  canonicalRepoPath,
    ...
    inFlightCycle = (async () => {
      try {
        ...
        const result = finding
          ? await advanceOneFinding(finding, { canonicalRepoPath, clock, deps })
          : { action: 'NOTHING_TO_DO', reason: 'no actionable finding' }
        onCycle(result)
      } catch (error) {
        onError(error)
      } finally {
