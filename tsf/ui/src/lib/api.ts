...
async function requestTolerant<TOk, TErr>(path: string, init?: RequestInit): Promise<TOk | TErr> {
  ...
  if (body === null) throw new ApiError(res.status, `Request failed: ${res.status}`)
  if (!res.ok && res.status >= 500) throw new ApiError(res.status, body.error ?? `Request failed: ${res.status}`)
...
export const api = {
  ...
  browseDirectory: (dirPath?: string) => request<DirectoryBrowseResult>(`/onboarding/browse${dirPath ? `?path=${encodeURIComponent(dirPath)}` : ''}`),
  analyzeRepo: (repoPath: string, handoffText: string) =>
    requestTolerant<OnboardingAnalysis, OnboardingAnalysisError>('/onboarding/analyze', { method: 'POST', body: JSON.stringify({ repoPath, handoffText }) }),
  commitOnboarding: (analysis: OnboardingAnalysis, addTo: { knownProjects?: boolean; activeFleet?: boolean; workSet?: boolean }) =>
    request<OnboardingCommitResult>('/onboarding/commit', { method: 'POST', body: JSON.stringify({ analysis, addTo }) }),
  refreshOnboardedProject: (projectId: string) => request<OnboardingRefreshResult>('/onboarding/refresh', { method: 'POST', body: JSON.stringify({ projectId }) })
}
