...
// Exhaustive Gauntlet V1): every symbol this section used to define its own
// copy of -- the adoption-verb allowlist, the "-n't" negation vocabulary,
// hasAdoptionVerb/negatesAdoptionVerb, classifyAdoptionCommandIntent itself
// -- now lives in ../domain/command-act-model.mjs, the ONE shared builder
// (see that module's own header). This file re-exports the same names so
// every existing consumer/test keeps working unchanged (RECONCILE BEFORE
// REPLACE -- no second, independently-drifting parser left alive).
// classifyAdoptionCommandIntent's real behavior is now provenance-aware
// (a quoted/reported "adopt X" grants no authority) and correction-aware
// ("Adopt A -- actually, don't." now correctly resolves to NOT_ADOPTION,
// and "final current-owner act wins" governs any later reversal) --
// see command-act-model.mjs's own resolveAdoptionActs/
// applyCorrectionAmendments for the fix itself; this file no longer
// contains any of that logic.
export {
  hasAdoptionVerb,
  negatesAdoptionVerb,
  classifyAdoptionCommandIntent,
  ADOPTION_VERB_PATTERN,
  NON_ADOPTION_ACCEPT_APPROVE_OBJECT,
  ADOPTION_CANDIDATE_NOUN_SOURCE,
  ARTICLE_SOURCE
} from './command-act-model.mjs'
