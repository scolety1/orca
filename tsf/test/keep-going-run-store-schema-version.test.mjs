...
const STATE_FILE = path.join(
  ...
  'server',
  '.local-state',
  `operator-state.test-keep-going-schema-version-${process.pid}.json`
)
...
test('keep going run store: schema-version guard at the real durable read boundary', async (t) => {
  try {
    await t.test(
      'a real run persists and re-reads cleanly (current version, no throw)',
      async () => {
    ...
    await t.test(
      ...
      async () => {
        ...
        opState.keepGoingRuns[PROJECT_ID] = {
          ...opState.keepGoingRuns[PROJECT_ID],
          schemaVersion: 'TSF_OVERNIGHT_RUN_V99_FROM_THE_FUTURE'
        }
        saveState(opState, { writerCollection: 'keepGoingRuns' })

        assert.throws(
          () => readKeepGoingRun(PROJECT_ID),
          (error) => {
