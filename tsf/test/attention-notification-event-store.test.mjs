...
const { createAttentionNotificationEvent, attentionNotificationEventIdFor } =
  await import('../domain/attention-notification-event.mjs')
...
test('attention notification event store', async (t) => {
  try {
    await t.test('registerAttentionNotificationEventIfAbsent creates on first call', async () => {
      ...
      assert.deepEqual(
        listAttentionNotificationEvents().map((e) => e.eventId),
        [event.eventId]
      )
    })

    await t.test(
      'a second call with the SAME eventId returns the existing record, created:false',
      async () => {
        ...
        assert.equal(
          listAttentionNotificationEvents().length,
          before,
    ...
    await t.test(
      'two near-simultaneous registrations for the same eventId never both create',
      async () => {
        ...
        assert.equal(
          createdCount,
          1,
          'exactly one of the two concurrent calls actually creates a new record'
        )
        assert.equal(a.event.eventId, b.event.eventId)
        const matching = listAttentionNotificationEvents().filter(
          (e) => e.eventId === a.event.eventId
        )
        assert.equal(
          matching.length,
          1,
          'never two durable records for the same content-addressed eventId'
        )
      }
    )

    await t.test(
      'withAttentionNotificationEvent: concurrent writers on the SAME eventId are serialized, not lost',
      async () => {
        ...
        const writers = Array.from({ length: 10 }, () =>
          withAttentionNotificationEvent(eventId, (current) => ({
            ...(current ?? {
              schemaVersion: 'TSF_ATTENTION_NOTIFICATION_EVENT_V1',
              eventId,
              count: 0
            }),
        ...
        assert.equal(
          final.count,
          10,
          'every one of 10 concurrent increments must land -- none silently lost'
        )
      }
    )

    await t.test(
      'version-guard rejects an unrecognized schemaVersion on read (fail-closed)',
      async () => {
        const { event } = await registerAttentionNotificationEventIfAbsent(() =>
          createAttentionNotificationEvent(
            eventFields({ sourceId: 'finding:version-guard-test' }),
            clock
          )
        )
        ...
        saveState(opState, { writerCollection: 'attentionNotificationEvents' })
        ...
        saveState(fresh, { writerCollection: 'attentionNotificationEvents' })
      }
    )

    await t.test(
      'a malformed/corrupt state value is handled the same way self-improvement-finding-store.mjs handles it (missing schemaVersion fails closed)',
      async () => {
        const opState = loadState()
