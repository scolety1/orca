...
const STATE_FILE = path.join(
  ...
  'server',
  '.local-state',
  `operator-state.test-planner-schema-version-${process.pid}.json`
)
process.env.TSF_UI_STATE_FILE = STATE_FILE

const { readPlannerMissionRecord, withPlannerMissionRecord, mutateCheckpoint } =
...
test('planner mission store: schema-version guard on the checkpoint at the real durable read boundary', async (t) => {
  try {
    await t.test(
      'a real checkpoint persists and re-reads cleanly (current version, no throw)',
      async () => {
        await mutateCheckpoint(
          MISSION_ID,
          () =>
            createPlannerMissionCheckpoint(
              { missionId: MISSION_ID, missionGoal: 'ship it', phase: 'BUILD', repoState },
              clock
            ),
          clock
        )
    ...
    await t.test(
      'a hand-corrupted checkpoint schemaVersion on disk is refused at read time, not silently operated on',
      async () => {
        ...
        opState.plannerMissions[MISSION_ID] = {
          ...opState.plannerMissions[MISSION_ID],
          checkpoint: {
            ...opState.plannerMissions[MISSION_ID].checkpoint,
            schemaVersion: 'TSF_PLANNER_MISSION_CHECKPOINT_V99_FROM_THE_FUTURE'
          }
        }
        saveState(opState, { writerCollection: 'plannerMissions' })

        assert.throws(
          () => readPlannerMissionRecord(MISSION_ID),
          (error) => {
    ...
    await t.test(
      'a lease with no checkpoint yet is never checked (nothing to version-guard) and never throws',
      async () => {
