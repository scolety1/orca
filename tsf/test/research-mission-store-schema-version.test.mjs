...
const STATE_FILE = path.join(
  ...
  'server',
  '.local-state',
  `operator-state.test-research-schema-version-${process.pid}.json`
)
...
test('research mission store: schema-version guard at the real durable read boundary', async (t) => {
  try {
    await t.test(
      'a real mission persists and re-reads cleanly (current version, no throw)',
      async () => {
    ...
    await t.test(
      'a hand-corrupted schemaVersion on disk is refused at read time, not silently operated on',
      async () => {
        ...
        opState.researchMissions[MISSION_ID] = {
          ...opState.researchMissions[MISSION_ID],
          schemaVersion: 'TSF_RESEARCH_MISSION_V99_FROM_THE_FUTURE'
        }
        saveState(opState, { writerCollection: 'researchMissions' })

        assert.throws(
          () => readResearchMission(MISSION_ID),
          (error) => {
