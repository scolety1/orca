...
open/deferred by explicit instruction. #22 was found live during the
`TSF_REAL_PROJECT_PILOT_READINESS_V1` mission's own rehearsal and is now
also Resolved, below -- a real fix landed same-day (`5399f43a74`) but this
ledger was never updated to reflect it; caught and reconciled during a
2026-09-19 dogfood pass, see that entry for detail.)*
...
| 22 | Command (`/command`), any per-project conversational question about a held project | BUG (P1) | Asking Command chat "Is `<project>` on hold?" or "Why is `<project>` waiting?" about a project under a REAL, active execution hold previously got a confident, false denial, because a genuine, non-imperative question about a named project classified as generic `GENERAL`/`QUESTION` and escalated to a live-LLM call (`invokeLivePlanner`) whose prompt-building never included the project's execution-hold record or canonical `primaryState`. **Was already fixed same-day this finding was recorded** (`5399f43a74`, 2026-09-16) via two layers: (1) a new, subject-agnostic `CANONICAL_STATUS_FACT_PATTERN` classifies this whole question family as deterministic `STATUS`, grounded directly in `project.primaryState`/`primaryReasonLabel`, never escalating to a live call at all; (2) defense in depth -- `live-planner.mjs`'s `operatorFacts` now always includes `ownerPrimaryState` (honestly `null` when absent) with an explicit prompt instruction to ground any hold/status claim in it, for whatever phrasing still does reach a live call. Regression tests: `test/live-planner-canonical-status-fact.test.mjs` (2 tests) plus extensions to `chat-responder`/`live-planner` test files (86 tests across the whole area, all green, re-run 2026-09-19). **This ledger row was simply never updated when the fix landed** -- caught and reconciled during a 2026-09-19 dogfood pass; no further code change was needed, the fix itself was real and complete. | commit `5399f43a74`; re-verified 2026-09-19 |
...
**#22 was the one real, disclosed exception to an otherwise clean
verdict**: a live-LLM-routed conversational question about a held
project's status could give a confident, false denial of the hold. Real,
...
**Fixed the same day** (`5399f43a74`) -- see the Resolved table above;
